function boxes = posedet_detect(im, model, thresh, background, postprocess)
...
pyra     = posedet_featpyramid(im,model);
interval = model.interval;
levels   = 1:length(pyra.feat);

if nargin >= 4 
    background = (double(background(:,:,1)) / 255);  
    if nargin < 5 || ~postprocess
        sbin = round(pyra.imy / (size(pyra.feat{1+pyra.interval}, 1) - pyra.pady*2));
        foreground_margin = 4;
        coeff =  1 / ((sbin * foreground_margin) ^ 2);
        [background, ~, ~] = posedet_dt(background, coeff, 0, coeff, 0);
        background = 1 - background;
        for rlevel = levels   
            y = (pyra.pady+2):(size(pyra.feat{rlevel}, 1)-pyra.pady);
            x = (pyra.padx+2):(size(pyra.feat{rlevel}, 2)-pyra.padx);
            pyra.feat{rlevel}(y, x, 32) = min(max(imresize(background, [length(y) length(x)]), 0), 1);
        end
        npart_mean = 0;
        for i = 1 : length(model.components)
            npart_mean = npart_mean + length(model.components{i});
        end
        npart_mean = npart_mean / length(model.components);
        for i = 1 : length(model.filters)
            model.filters(i).w(:, :, 32) = -2 / (npart_mean * size(model.filters(i).w, 1) * size(model.filters(i).w, 2));
        end
    else
        sbin = round(pyra.imy / (size(pyra.feat{1+pyra.interval}, 1) - pyra.pady*2));
        margin = 2;
        coeff =  1 / (sbin * sbin * margin);
        [background, ~, ~] = posedet_dt(background, coeff, 0, coeff, 0); % fills the holes and enlarge a bit
    end
end
  ...
  for c = 1:length(model.components),
    ...
    for k = 1:numparts,
      ...
      if isempty(resp{level}),
        resp{level} = posedet_fconv(pyra.feat{level},filters,1,length(filters));
      end      
      for fi = 1:length(f)
        if nargin >= 5 && postprocess
            bg = max(min(imresize(background, [size(resp{level}{f(fi)}, 1), size(resp{level}{f(fi)}, 2)]), 1), exp(-0.05));            
            parts(k).score(:,:,fi) = resp{level}{f(fi)} + log(bg);            
        else
...
function [score,Ix,Iy,Ik] = passmsg(child,parent)
  ...
  for k = 1:K
    [score_tmp,Ix_tmp,Iy_tmp] = posedet_dt(child.score(:,:,k), child.w(1,k), child.w(2,k), child.w(3,k), child.w(4,k));
