function pyra = posedet_featpyramid(im, model)
...
for i = 1:interval
  scaled = posedet_resize(im, 1/sc^(i-1));
  pyra.feat{i} = posedet_features(scaled,sbin);
  pyra.scale(i) = 1/sc^(i-1);
  % remaining interals
  for j = i+interval:interval:max_scale
    scaled = posedet_reduce(scaled);
    pyra.feat{j} = posedet_features(scaled,sbin);
