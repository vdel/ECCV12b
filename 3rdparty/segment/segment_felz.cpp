...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  if (nrhs != 4 || nlhs != 1) {
	mexErrMsgTxt("usage: segs = segment(sigma, k, min, img)\n");
  }
  
  if (mxGetClassID(prhs[3]) != mxUINT8_CLASS || mxGetNumberOfDimensions(prhs[3]) != 3) {
    mexErrMsgTxt("'img' should be a 3D array of type UINT8.");  
  }

  float sigma = mxGetScalar(prhs[0]);
  float k = mxGetScalar(prhs[1]);
  int min_size = mxGetScalar(prhs[2]);
  
  const mwSize *d = mxGetDimensions(prhs[3]);
  int h = d[0];
  int w = d[1];
  unsigned char *ptr = (unsigned char*)mxGetPr(prhs[3]);
